...
- 摘要提示词按原始位置携带真实图片块，只有当前模型不支持图片时才退化为 `[image <mime>, about <大小>KB omitted]` 占位符；`context_get` 文本始终用占位符，base64 不进任何提示词。entry token 记账走 Pi 的消息投影（`sessionEntryToContextMessages` + `estimateTokens`），摘要请求预算按实际提示词内容估算。
